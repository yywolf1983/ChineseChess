...
public class MoveSimulator {
    ...
    public ChessInfo simulateMove(ChessInfo info, String moveString, boolean isRed) {
        ...
        if (moveString != null && !moveString.isEmpty()) {
            Utils.LogUtils.d("MoveSimulator", "开始解析走法: " + moveString + ", isRed=" + isRed);
            // 标准化走法字符串：全角转半角 + 红方中文数字/黑方阿拉伯数字
            String normalizedMoveString = ChessNotationTranslator.normalizeMoveString(moveString, isRed);
            ...
            if (isSpecialMove) {
                // 处理特殊走法
                Utils.LogUtils.d("MoveSimulator", "开始处理特殊走法: " + normalizedMoveString);
                String specialMark = "";
                ...
                if (specialCharIndex != -1) {
                    ...
                    Utils.LogUtils.d("MoveSimulator", "特殊走法处理 - 特殊标记: " + specialMark + ", 基础棋子名称: " + basePieceName + ", 剩余部分: " + rest);
                    ...
                    if (pieceType != -1) {
                                ...
                                if (newInfo.piece[y] != null) {
                                    for (int x = 0; x < 9; x++) {
                                        ...
                                        if (piece == pieceType && isSameColor) {
                                            ...
                                            Utils.LogUtils.d("MoveSimulator", "收集到棋子: 位置= " + x + "," + y + ", 类型= " + piece);
                                        }
                                    }
                                }
                            }
                        ...
                        Utils.LogUtils.d("MoveSimulator", "收集到的棋子数量: " + piecePositions.size());
                        ...
                        if (!piecePositions.isEmpty()) {
                            ...
                            if (targetX != null) {
                                Utils.LogUtils.d("MoveSimulator", "目标列棋盘坐标: " + targetX);
                                // 检查能到达目标列的列
                                for (java.util.Map.Entry<Integer, java.util.List<Pos>> entry : columnPieces.entrySet()) {
                                    ...
                                    Utils.LogUtils.d("MoveSimulator", "检查列 " + col + "，棋子数量: " + colPieces.size());
                                    for (Pos pos : colPieces) {
                                        ...
                                        if (possibleMoves != null) {
                                            Utils.LogUtils.d("MoveSimulator", "棋子位置 " + pos.x + "," + pos.y + " 的可能走法数量: " + possibleMoves.size());
                                            for (Pos move : possibleMoves) {
                                                Utils.LogUtils.d("MoveSimulator", "可能的移动位置: " + move.x + "," + move.y);
                                                if (move.x == targetX) {
                                                    selectedColumn = col;
                                                    Utils.LogUtils.d("MoveSimulator", "找到能到达目标列的棋子，列: " + col + "，位置: " + pos.x + "," + pos.y);
                                                    break;
                                                }
                                            }
                                        }
                            ...
                            if (specialMark.equals("前") || specialMark.equals("后") || specialMark.equals("中")) {
                                Utils.LogUtils.d("MoveSimulator", "处理前中后标记: " + specialMark);
                                // 首先检查是否有任何一列有多个相同的棋子
                                boolean hasAnyColumnWithMultiplePieces = false;
                                for (java.util.Map.Entry<Integer, java.util.List<Pos>> entry : columnPieces.entrySet()) {
                                    if (entry.getValue().size() > 1) {
                                        hasAnyColumnWithMultiplePieces = true;
                                        Utils.LogUtils.d("MoveSimulator", "找到有多个棋子的列: " + entry.getKey() + "，棋子数量: " + entry.getValue().size());
                                        break;
                                    }
                                }
                                ...
                                if (targetPiecePos != null) {
                                    ...
                                    if (hasAnyColumnWithMultiplePieces) {
                                        Utils.LogUtils.d("MoveSimulator", "优先处理有多个棋子的列");
                                        // 遍历所有列，找到有多个棋子的列
                                        for (java.util.Map.Entry<Integer, java.util.List<Pos>> entry : columnPieces.entrySet()) {
                                            int col = entry.getKey();
                                            java.util.List<Pos> colPieces = entry.getValue();
                                            if (colPieces.size() > 1) {
                                                ...
                                                // 打印排序后的棋子位置
                                                Utils.LogUtils.d("MoveSimulator", "排序后棋子位置:");
                                                for (int i = 0; i < colPieces.size(); i++) {
                                                    Pos pos = colPieces.get(i);
                                                    Utils.LogUtils.d("MoveSimulator", "位置 " + i + ": " + pos.x + "," + pos.y);
                                                }
                                                
                                                if (specialMark.equals("前")) {
                                                    // 前：相对己方，离对方底线近的棋子
                                                    targetPiecePos = colPieces.get(0);
                                                    Utils.LogUtils.d("MoveSimulator", "选择前棋子: " + targetPiecePos.x + "," + targetPiecePos.y);
                                                } else if (specialMark.equals("后")) {
                                                    // 后：相对己方，离己方底线近的棋子
                                                    targetPiecePos = colPieces.get(colPieces.size() - 1);
                                                    Utils.LogUtils.d("MoveSimulator", "选择后棋子: " + targetPiecePos.x + "," + targetPiecePos.y);
                                                } else if (specialMark.equals("中")) {
                                                    // 中：中间位置的棋子
                                                    targetPiecePos = colPieces.get(colPieces.size() / 2);
                                                    Utils.LogUtils.d("MoveSimulator", "选择中棋子: " + targetPiecePos.x + "," + targetPiecePos.y);
                                                }
                                                ...
                                                if (targetPiecePos != null && targetX != null) {
                                                    ...
                                                    if (possibleMoves != null) {
                                                        Utils.LogUtils.d("MoveSimulator", "选择的棋子可能走法数量: " + possibleMoves.size());
                                                        for (Pos move : possibleMoves) {
                                                            Utils.LogUtils.d("MoveSimulator", "可能的移动位置: " + move.x + "," + move.y);
                                                            if (move.x == targetX) {
                                                                Utils.LogUtils.d("MoveSimulator", "找到能移动到目标列的棋子");
                                                                // 找到能移动到目标位置的棋子，停止搜索
                                                                break;
                                                            }
                                                        }
                                    ...
                                    if (targetPiecePos == null && targetX != null) {
                                        Utils.LogUtils.d("MoveSimulator", "没有找到合适的棋子，尝试找到能移动到目标位置的棋子");
                                        for (Pos pos : piecePositions) {
                                            int piece = newInfo.piece[pos.y][pos.x];
                                            java.util.List<Pos> possibleMoves = Rule.PossibleMoves(newInfo.piece, pos.x, pos.y, piece);
                                            if (possibleMoves != null) {
                                                for (Pos move : possibleMoves) {
                                                    if (move.x == targetX) {
                                                        targetPiecePos = pos;
                                                        Utils.LogUtils.d("MoveSimulator", "找到能移动到目标位置的棋子: " + pos.x + "," + pos.y);
                                                        break;
                                                    }
                                                }
                                            }
                            ...
                            Utils.LogUtils.d("MoveSimulator", "选择的目标棋子位置: " + (targetPiecePos != null ? targetPiecePos.x + "," + targetPiecePos.y : "null"));
                            ...
                            if (targetX != null && targetPiecePos != null) {
                                ...
                                if (!canReachTarget && !(specialMark.equals("前") && basePieceName.equals("卒"))) {
                                    Utils.LogUtils.d("MoveSimulator", "前卒特殊处理，不自动切换棋子");
                                    for (Pos pos : piecePositions) {
                                        ...
                                        if (otherPossibleMoves != null) {
                                            for (Pos otherPos : otherPossibleMoves) {
                                                if (otherPos.x == targetX) {
                                                    targetPiecePos = pos;
                                                    Utils.LogUtils.d("MoveSimulator", "切换到能到达目标列的棋子: " + pos.x + "," + pos.y);
                                                    canReachTarget = true;
                                                    break;
                                                }
                                            }
                                ...
                                if (possibleMoves != null) {
                                    for (Pos targetPos : possibleMoves) {
                                        ...
                                        if (generatedMove != null) {
                                            // 标准化走法：全角转半角 + 红方中文数字/黑方阿拉伯数字
                                            normalizedGeneratedMove = ChessNotationTranslator.normalizeMoveString(generatedMove, isRed);
                                        } else {
                            ...
                            if (targetPiecePos != null) {
                                int piece = newInfo.piece[targetPiecePos.y][targetPiecePos.x];
                                Utils.LogUtils.d("MoveSimulator", "目标棋子类型: " + piece + ", 位置: " + targetPiecePos.x + "," + targetPiecePos.y);
                                // 生成该棋子的可能走法
                                java.util.List<Pos> possibleMoves = Rule.PossibleMoves(newInfo.piece, targetPiecePos.x, targetPiecePos.y, piece);
                                Utils.LogUtils.d("MoveSimulator", "目标棋子的可能走法数量: " + (possibleMoves != null ? possibleMoves.size() : 0));
                                if (possibleMoves != null) {
                                    // 尝试找到与走法字符串匹配的移动
                                    for (Pos targetPos : possibleMoves) {
                                        // 生成走法字符串并与输入进行比较
                                        String generatedMove = generateMoveString(newInfo, piece, targetPiecePos, targetPos, isRed);
                                        // 标准化生成的走法字符串以进行比较
                                        String normalizedGeneratedMove = generatedMove != null ? ChessNotationTranslator.normalizeMoveString(generatedMove, true) : null;
                                        
                                        Utils.LogUtils.d("MoveSimulator", "比较 - 生成的走法: " + normalizedGeneratedMove + ", 目标走法: " + baseMoveString + ", 目标位置: " + targetPos.x + "," + targetPos.y);
                                        
                                        // 检查是否是横向移动（包含"平"）
                                        if (normalizedGeneratedMove != null && baseMoveString.contains("平")) {
                                            // 统一数字格式：将中文数字转换为阿拉伯数字
                                            String normalizedGeneratedMoveForCompare = Info.ChessPiece.chineseToArabic(normalizedGeneratedMove);
                                            String baseMoveStringForCompare = Info.ChessPiece.chineseToArabic(baseMoveString);
                                            ...
                                            if (generatedPingIndex != -1) {
                                                ...
                                                if (basePingIndex != -1) {
                                                    ...
                                                    Utils.LogUtils.d("MoveSimulator", "简化比较 - 生成: 棋子=" + generatedPieceName + " vs " + basePiece + ", 目标列: " + generatedTargetCol + " vs " + baseTarget);
                                                    
                                                    // 比较棋子名称和目标列号
                                                    if (generatedPieceName.equals(basePiece) && generatedTargetCol.equals(baseTarget)) {
                                                        // 找到匹配的走法，执行移动
                                                        Utils.LogUtils.d("MoveSimulator", "找到特殊走法匹配，执行移动: 从 " + targetPiecePos.x + "," + targetPiecePos.y + " 到 " + targetPos.x + "," + targetPos.y);
            ...
            Utils.LogUtils.d("MoveSimulator", "棋子名称: " + pieceName + ", 棋子类型: " + targetPieceType + ", isRed: " + isRed);
                        ...
                        for (int x = 0; x < 9; x++) {
                            ...
                            if (piece != 0) {
                                ...
                                if (isCurrentSide && piece == targetPieceType) {
                                    ...
                                    Utils.LogUtils.d("MoveSimulator", "找到候选棋子: 位置= " + x + "," + y + ", 类型= " + piece);
                                }
                            }
                        }
                    }
            ...
            Utils.LogUtils.d("MoveSimulator", "候选棋子数量: " + candidatePieces.size());
            ...
            for (Pos pos : candidatePieces) {
                ...
                if (possibleMoves != null) {
                    ...
                    for (Pos targetPos : possibleMoves) {
                        ...
                        Utils.LogUtils.d("MoveSimulator", "检查走法: 生成走法= " + normalizedGeneratedMove + ", 目标走法= " + normalizedMoveString);
                        ...
                        if (isMatch) {
                            // 找到匹配的走法，执行移动
                            Utils.LogUtils.d("MoveSimulator", "找到匹配的走法，执行移动: 从 " + x + "," + y + " 到 " + targetPos.x + "," + targetPos.y);
    ...
    private String getChineseNumber(int number) {
        return Info.ChessPiece.toChineseNumber(number);
    }
}