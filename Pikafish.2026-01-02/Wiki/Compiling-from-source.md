...
On Unix-like systems, it should be easy to compile Pikafish directly from the source code with the included Makefile in the folder `src`.
...
3. Navigate to wherever you saved the script (e.g. type and execute `cd '/d/Program Files/Pikafish'` to navigate to `D:\Program Files\Pikafish`).
...
_Note: Building Pikafish with Visual Studio is **not officially supported**._
...
9. Run bench with the instrument build(very slow) and quit, it should generate "Pikafish.pgd" and "Pikafish!1.pgc".
...
2. Build Pikafish writing and executing this bash script
<details><summary>Click to view</summary>
...
to compile Pikafish out of the box, and this is the method used by default
...
But it is quite possible to compile and run Pikafish on older versions of macOS! Below
...
3) Go to the Pikafish "src" directory, then try a default build and run Pikafish:
...
to compile Pikafish. If not, continue with step 5 to install GNU gcc10 :-)
...
7) You can now go back to the "src" directory of Pikafish, and try to build
Pikafish by pointing at the right compiler:
...
8) Enjoy Pikafish on macOS!

See [this pull request](https://github.com/official-pikafish/Pikafish/pull/3049) for further discussion.

---

## For Android

You can build Pikafish for your ARM CPU based mobile Android device,
using the Pikafish supplied Makefile.
...
we will cover these in this example. Before you try to build Pikafish, make sure
...
Furthermore you should be aware of the fact that Pikafish is just the chess engine.
...
"talks" to the Pikafish chess engine so that it analyses the chess position in a game
...
Now we will checkout the Pikafish source code with git, and start the build. The steps for Windows in the MSYS environment
session and for Linux are now basically the same. In your home directory, make subdirectories for the git checkout:

```bash
$ mkdir -p ~/repos/official-pikafish
$ cd ~/repos/official-pikafish
...
.
.
aarch64-linux-android21-clang++ -o pikafish benchmark.o bitbase.o bitboard.o endgame.o evaluate.o main.o material.o misc.o movegen.o movepick.o pawns.o position.o psqt.o search.o thread.o timeman.o tt.o uci.o ucioption.o tune.o tbprobe.o evaluate_nnue.o half_ka_v2_hm.o  -static-libstdc++ -pie -lm -latomic -Wall -Wcast-qual -fno-exceptions -std=c++17  -stdlib=libc++ -fPIE -DUSE_PTHREADS -DNDEBUG -O3 -fexperimental-new-pass-manager -DIS_64BIT -DUSE_POPCNT -DUSE_NEON=8 -flto
make[1]: Leaving directory '/home/johndoe/repos/official-pikafish/Pikafish/src'
```

You will now have a binary file called `pikafish` in your current directory. Check with the file command that
it is indeed an ARM binary for Android:

```bash
$ file pikafish
pikafish: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, with debug_info, not stripped
```
...
aarch64-linux-android-strip pikafish
```
...
```bash
$ file pikafish
pikafish: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, stripped
```
...
So in Droidfish you will be able to see that you are running your self compiled DEVelopment version of Pikafish, and not the built-in
version, which is substantially older.

Voilà: you have your Android build of Pikafish. What is now left to do is to copy it over to your smartphone or tablet,
...
create a Windows network share, copy the Pikafish binary into that share directory, and then on your Android
device use the File Manager app to connect to that network share and copy the Pikafish binary over to the
Droidfish uci (engines) directory.

In Droidfish, open the left menu, find the Engines management submenu, pick the `pikafish_DEV_armv8` binary,
...
_Note: As Pikafish is a very computation-intense program, you should probably not give it as many threads as
your device CPU has processor cores. Especially in Analysis mode, when Pikafish is thinking permanently, and for
...
```bash
#!/bin/bash
# functions to build Pikafish
_build_pf () {
make build ARCH=x86-64$1 COMP=mingw -j
make strip COMP=mingw
mv pikafish.exe ../../pikafish-x64${1}.exe
make clean COMP=mingw
}

_build_pf_pgo () {
make profile-build ARCH=x86-64$1 COMP=mingw PGOBENCH="wine ./pikafish.exe bench" -j
make strip COMP=mingw
mv pikafish.exe ../../pikafish-x64${1}-pgo.exe
make clean COMP=mingw
}
...
# clone Pikafish source code
git clone --single-branch --branch master https://github.com/official-pikafish/Pikafish.git
cd Pikafish/src

# build Pikafish executables
...
if [ "$(x86_64-w64-mingw32-c++-posix -Q -march=native --help=target | grep mpopcnt | grep enabled)" ] ; then
  _build_pf_pgo -sse41-popcnt
else
  _build_pf -sse41-popcnt
fi
...
Here is a script to cross compile from a clean Ubuntu a static build of Pikafish targeting an armv8 or armv7 CPU running on Linux or Android:
...
qemu-aarch64 pikafish compiler
qemu-aarch64 pikafish bench
...
# test: qemu's magic at work
qemu-arm pikafish compiler
qemu-arm pikafish bench

```
</details>

Here is a script to cross compile from a msys2 msys/mingw-w64 shell a static build of Pikafish targeting an armv8 or armv7 CPU running on Linux or Android:
...
cd Pikafish/src
make -j build ARCH=armv8 COMP=gcc CXX="zig c++ -target aarch64-linux-musl"
mv pikafish.exe pikafish_armv8
```
</details>
